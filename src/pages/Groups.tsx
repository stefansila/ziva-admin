...
interface GroupData {
    ...
    name: string;
    patientCount: number;
    patients: CombinedUserProfile[];
}

const Groups: React.FC = () => {
    const [activeGroup, setActiveGroup] = useState<string>('unassigned');
    ...
    const fetchGroupsData = async () => {
        try {
            setIsLoading(true);
            const users = await userService.getCombinedUserProfiles();
            
            // Initialize groups
            const groups: Record<string, CombinedUserProfile[]> = {
                'unassigned': [],
                'high': [],
                'moderate': [],
                'average': [],
            ...
            // Sort users into groups based on their health profile risk group
            users.forEach(user => {
                const riskGroup = user.healthProfile?.riskGroup;
                if (!riskGroup) {
                    groups['unassigned'].push(user);
                } else if (groups[riskGroup]) {
                    groups[riskGroup].push(user);
                }
            });
            ...
            const formattedGroups: GroupData[] = [
                {
                    id: 'unassigned',
                    name: 'Unassigned',
                    patientCount: groups['unassigned'].length,
                    patients: groups['unassigned']
                },
                {
                    id: 'high',
                    name: 'High Risk',
                    patientCount: groups['high'].length,
                    patients: groups['high']
                },
                {
                    id: 'moderate',
                    name: 'Moderate Risk',
                    patientCount: groups['moderate'].length,
                    patients: groups['moderate']
                },
                {
                    id: 'average',
                    name: 'Average Risk',
                    patientCount: groups['average'].length,
                    patients: groups['average']
                },
                {
                    id: 'control',
                    name: 'Control Group',
                    patientCount: groups['control'].length,
            ...
            showNotification('error', 'Failed to load groups', error instanceof Error ? error.message : 'Please try again');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const filteredPatients = activeGroupData?.patients.filter(patient =>
        getDisplayName(patient).toLowerCase().includes(searchTerm.toLowerCase()) ||
                                            ...
                                            filteredPatients.map(patient => (
                                                ...
                                                >
                                                    <div className="patient-card-header">
                                                        <img 
                                                            src={generateAvatar(getDisplayName(patient))} 
                                                            alt={getDisplayName(patient)}
                                                            className="patient-avatar"
                                                        />
                                                        <div className="patient-info">
                                                            <h4 className="patient-name">{getDisplayName(patient)}</h4>
                                                    ...
                                                    <div className="patient-details">
                                                        <div className="patient-detail-row">
                                                            ...
                                                            <span className="patient-detail-value">{calculateAge(patient.dateOfBirth) || 'N/A'}</span>
                                                            <span className="patient-detail-label">Gender:</span>
                                                            <span className="patient-detail-value">{patient.gender || 'N/A'}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ))
